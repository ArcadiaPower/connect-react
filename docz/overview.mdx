...
This package is a React wrapper around Arcadia's credential sync component. It provides two ways to integrate the component into your React application - via hooks and via HoCs (higher order components).
...
```
npm i @arcadia-eng/credential-sync-react
```

Via yarn:

```
yarn add @arcadia-eng/credential-sync-react
```
...
export default withCredentialSync(CreateCredentials);
```
